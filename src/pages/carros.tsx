  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        ...
        if (searchParams.cidade) query.append("cidade", searchParams.cidade);
        ...
        // Garantir que a faixa de preço seja sempre passada corretamente
        query.append("precoMinimo", String(priceRange[0]));
        query.append("precoMaximo", String(priceRange[1]));
        
        // Adicionar o parâmetro sortBy
        query.append("sortBy", sortByRecent ? "recente" : "antigos");
        
        query.append("page", String(currentPage));
        query.append("limit", "9");

        const apiUrl = `/api/cars?${query.toString()}`;
        console.log("URL gerada:", apiUrl);
